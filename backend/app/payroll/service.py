...
# Both keep a plain imported binding here anyway: Day Grid (save_day_grid)
...
# resolve those names. This plain imported binding still stays here: Day
# Grid (save_day_grid) still lives in this module and still calls
# _lock_pay_item_for_source_write by its bare name, in the same call order
# as before (pay-item lock, then _lock_period_for_mutation) — unchanged by
# this move, since ordering is enforced by each caller, not by the function
# itself.
...
# _upsert_entry_state, _validate_status_key, and _enforce_status_key_limits
# keep a plain imported binding here: Day Grid (save_day_grid) still lives
# in this module and still calls all three by their bare names — these
# bindings are load-bearing for that runtime caller, not incidental.
...
# its own copy directly from app.payroll.period_day_calendar. This plain
# imported binding still stays here: Day Grid (get_day_grid, save_day_grid)
# still lives in this module and still calls it by its bare name — this
