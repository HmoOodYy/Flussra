...
# every existing internal call site (Rates, Driver Pay Rules, lifecycle,
# finalization, calculation, Day Grid, Current Payroll Hub) and the
# test_cp4f_finalization_snapshot_projection.py monkeypatch keep working
# unchanged.
...
#
# Stage B4-2C correction: _write_pay_rule_audit IS re-exported here, unlike
# the other private helpers, because copy_driver_rates (a Rates function
# that stayed in this file) still calls it directly at its original call
# site -- a live production caller outside the Driver Pay Rules cluster that
# B4-2B's discovery missed (it searched only within the cluster's own line
# range). Without this re-export, copy_driver_rates(..., include_pay_rules=True)
# raises NameError.
from app.payroll.driver_pay_rules import (
    _write_pay_rule_audit,
...
# create_period_from_candidate (CP-1C) still call these directly from this
# module. _build_branch_entry (Current Payroll Hub) moved to
# app.payroll.current_hub in Stage B4-3B and now imports _check_slot_matrix
# directly from period_creation instead of through this facade.
