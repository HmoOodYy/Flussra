...
P6A implements the finalized Overview and calculation report routes below. P6B implements finalized Off/Status history, P6C implements finalized Rates Used, and P6D implements finalized immutable audit/security detail.
...
18. Finalized information-library metadata beyond the completed immutable-evidence foundation, P6A overview/report contracts, P6B Off/Status history, P6C Rates Used/Bonus-used history, and P6D audit/security detail remains deferred: P6E participant/role read and P6F correction-boundary library contracts are not started.
...
5. `ledger.view` and `ledger.audit.view` are seeded. P6A/P6B/P6C enforce `ledger.view`; P6D enforces the additive `ledger.view` plus `ledger.audit.view` audit contract.
...
| Phase 6 | Finalized payroll information library and audit | In Progress | Claude | Codex ledger immutability review |
...
- [x] P6D: security/audit detail.
- [ ] P6E: participant/role snapshot (deferred).
- [ ] P6F: future correction boundary contract (deferred).
...
#### Internal immutable-evidence foundation — Complete

Implementation commit: `de3d8e8` — `feat: add phase 6 immutable evidence foundation` (`8 files changed, 1431 insertions(+), 12 deletions(-)`); Alembic head is `0064`. Independent review: `PASS_WITH_NOTES`; P0 none; P1 none; `SAFE_TO_COMMIT_PHASE6_FOUNDATION`.

Migration `0064` adds `PayrollPeriodWorkflowActionEvidence`, `PayrollCalculationSnapshotUsedRateDefinitions`, a nullable `PayrollCalculationSnapshotLines.UsedRateDefinitionID` provenance link, scoped foreign keys, uniqueness, targeted indexes, scope guards, immutable triggers, and catalog seeds for `ledger.view` and `ledger.audit.view`. It grants neither permission by default and implements no public route enforcement.

Participant evidence is historical explanation only, not workflow authority or a lifecycle status. It captures `SUBMITTED`, `RESUBMITTED`, `REVIEW_APPROVED`, `REVIEW_EDIT_REQUESTED`, `REVIEW_REJECTED`, and `FINALIZED` inside their authoritative Submit/Resubmit, review-decision, and Finalize transactions. Actor identity, display identity, relevant responsibility context, timestamp, and snapshot/review provenance are frozen by value; later User or Role changes cannot rewrite them, and UPDATE/DELETE is blocked.

Used-rate/rule evidence is descriptive provenance only, not a calculation engine or financial authority. CP-4D captures only referenced `DriverRate` and `DriverPayRule` definitions in the same snapshot transaction, fingerprint-deduplicates them within each revision, and links applicable snapshot lines deterministically. Legitimate no-rate lines remain unlinked. Later live changes cannot alter prior evidence, and Resubmit produces independent revision evidence. `PayrollFinalLines` remains the sole Locked/Archived money authority; CP-4B/CP-4D/CP-4E/CP-4F financial/lifecycle authority and CP-5C behavior remain unchanged.
...
P6A itself did not implement `GET /payroll/finalized/{period_id}/off-drivers`, `GET /payroll/finalized/{period_id}/rates-used`, or `GET /payroll/finalized/{period_id}/audit`; P6B subsequently supplied finalized Off/Status history, P6C subsequently supplied Rates Used/Bonus-used history, and P6D subsequently supplied finalized audit/security detail. P6E-P6F and Phase 7 remain deferred.

#### P6B — Finalized Off/Status source snapshot views — Complete

Implementation commit: `0a7d412` — `feat: add finalized off status history` (`4 files changed, 880 insertions(+), 3 deletions(-)`). It adds `GET /payroll/finalized/{period_id}/off-drivers`; Alembic remains `0064`, with no migration `0065`, frontend, selected-day finalized route, correction workflow, cache, or Phase 7 work. Independent review: `PASS_WITH_NOTES`; P0 none; P1 none; `SAFE_TO_COMMIT_P6B`.

P6B is a read-only Locked/Archived projection. It resolves the one exact calculation snapshot identified by finalized `SourceSnapshot` provenance; it never selects a latest/newest revision or reconstructs history from mutable state. Missing or inconsistent provenance leaves dependent P6B evidence `UNAVAILABLE`. Status remains not a PayItem: historical facts come from immutable `PayrollCalculationSnapshotStatusEntries`; `PayrollPeriodDays` supplies the frozen calendar and `PayrollPeriodDriverEligibility` supplies the historical eligible roster. Current StatusKey, Driver/Employee/employment, schedule, and DraftLine state are never used as finalized truth.

Fully-Off remains the CP-5B whole-period rule: every eligible scheduled workday must have frozen Off Status and no immutable normal operational work/activity. Bonus-only, Status-payment-only, system/min/max-only, and already-defined period-financial-only evidence do not disqualify it. New versioned evidence with zero Status rows is `AVAILABLE` and `EMPTY`; legacy missing evidence is `UNAVAILABLE`, while P6A FinalLines money remains independently available. Historical driver identity uses immutable eligibility/snapshot identity where available; accepted nullable display identity for Status rows outside that eligible identity remains unchanged.

The route requires `ledger.view`; `payroll.view` or `ledger.audit.view` alone are insufficient. It preserves tenant/company/branch scope and fails closed for Driver and `OwnDriverDataOnly`; non-finalized states fail closed. Batched period/snapshot reads load only finalized context/provenance, calendar, eligibility, frozen Status, and immutable work evidence, with no per-driver/date/Status N+1 path or intentional GET write. Focused evidence: P6B 6 passed; CP-5B 12; P6A 7; immutable evidence 3; CP-4D 27; CP-4E 12; CP-4F 8; frozen Status/Bonus 8; CP-5C reports 24; permission catalog 15; branch access 11; Ruff and compile passed; `0064` is the sole Alembic head.
...
#### P6D — Finalized audit/security detail — Complete with Notes

Implementation commit: `04b0a4c` — `feat: add finalized payroll audit evidence` (`16 files changed, 2512 insertions(+), 271 deletions(-)`), including migration `0065`; Alembic head is `0065`. P6D adds `GET /payroll/finalized/{period_id}/audit` as a lazy, read-only Locked/Archived finalized-library route. Independent final review: `PASS_WITH_NOTES`; P0 none; P1 none; `SAFE_TO_COMMIT_P6D`.

P6D is a backend-owned immutable payroll audit/security history. It reads period-scoped immutable P6D audit evidence, immutable workflow participant evidence, exact snapshot membership, and deterministic finalized provenance. It does not reconstruct historical meaning from mutable current `PayrollDraftLines`, daily entry state, `PayrollBonusEvents`, Users, Roles, PayItems, StatusKeys, current rates/rules, or other live configuration. Mutable rows may be read only at an authoritative capture transaction to freeze the values P6D preserves. P6D does not calculate payroll money; `PayrollFinalLines` remains finalized money authority.

Chronology is whole-period history, grouped by Submit/Resubmit snapshot revision. Returned correction and Resubmit retain earlier immutable revision history rather than rewriting it; events retain exact snapshot membership. The final approved/finalized revision resolves only from immutable finalized provenance, never `MAX(snapshot)` or a latest-created shortcut. Review-comment evidence validates the PeriodApproval request type, company, branch, payroll period, exact calculation snapshot, and event/snapshot membership; wrong-period, wrong-type, snapshot-less, wrong-snapshot, and missing-membership cases are rejected by database integrity rules.

Migration `0065` provides period coverage and event/membership evidence with scoped restrictive relationships and direct evidence UPDATE/DELETE rejection. Once a PayrollPeriod has P6D immutable audit evidence, it cannot be physically deleted regardless of its current lifecycle Status. The durable boundary is evidence existence itself: no `EverLeftDraft`, `EverOpened`, or other lifecycle-history marker exists. The adversarial `evidence exists -> status is directly tampered back to Draft -> parent PayrollPeriod DELETE` path is rejected, and the evidence remains. An unevidenced, unsnapshotted Draft period may still use the supported physical cleanup path. Downgrade refuses when immutable P6D evidence exists; there is no historical backfill or fabricated legacy history. Coverage reports `AVAILABLE`, `EMPTY`, `PARTIAL`, or `UNAVAILABLE` without mutable fallback.

The route requires finalized-library `ledger.view` plus `ledger.audit.view`; `payroll.view` is not a substitute. It preserves Locked/Archived-only access, company isolation, branch access enforcement, foreign-company non-leakage, Driver denial, and `OwnDriverDataOnly` denial. Its period/snapshot-scoped reads are batched and set-based, with no per-driver/event/status/Bonus/revision read loop identified.

Final validation evidence: P6D focused 24 passed, 1 approved N/A skip; P6 foundation + P6A + P6B + P6C 26 passed; CP-4D + CP-4E + CP-4F 47 passed; Review + CP-6 Review 51 passed; Ledger + Finalization Preview + CP-5C frozen evidence + CP-5C reports 79 passed; Permission catalog + branch access 26 passed; CP-3A + CP-3B2b 67 passed; CP-3B1 + CP-3B2a 50 passed; CP-2D2 45 passed. Focused Ruff and py_compile passed; Alembic `0065` is the current head. This is representative targeted validation, not a claim that the complete backend suite passed.

Accepted non-blocking P2/P3 notes remain same-process Bonus-suite fixture/Open-slot contamination and environment-only LF/CRLF, user-level Git-ignore, and test-environment cleanup advisories. They do not reopen P6D.

**Roadmap freeze after P6D:** P6D is the stopping boundary for the current Phase 6 implementation sequence. P6E and P6F are deferred, and Phase 7 implementation is deferred; Phase 6 remains `In Progress` and Phase 7 remains `Pending` while both are temporarily frozen pending the next approved roadmap. They are not cancelled and their existing planning content remains. The project is intentionally moving to the separate [Post-P6D Cleanup and Product Readiness Plan](POST_P6D_CLEANUP_AND_PRODUCT_READINESS_PLAN.md) before returning to this deferred backend roadmap; that document owns current execution priority and does not replace this plan's historical architecture. A newly discovered P0/P1 involving payroll correctness, financial authority, tenant/company/branch isolation, security, immutable finalized history, or data corruption may still be addressed immediately.
...
**Audit and Phase 7 boundary:** P6D presents the immutable actor/time/reason/security facts the finalized library promises. Phase 7 retains broad append-only audit hardening, constraint validation, permission cleanup, load/performance validation, migration rehearsal, rollout, and reconciliation. The completed `0064` foundation, P6A, P6B, P6C, and P6D started no frontend or Phase 7 scope. P6E/P6F and Phase 7 implementation are deferred under the roadmap freeze recorded above.

#### Phase 6 architecture plan

**Architecture decision:** Phase 6 is a locked/archived read library. `PayrollFinalLines` remains the sole authority for finalized money. The originating immutable calculation snapshot explains a FinalLine's source, rate, Status, Bonus, and period metadata, but never supersedes the FinalLine amount. No Phase 6 reader may call LIVE calculation, resolve a current rate, load a current PayItem/Status/User label as historical truth, or read mutable DraftLines to fill a missing historical field.

**Deterministic finalized provenance:** Resolve the originating snapshot from the `payroll_calculation_snapshot_id`, `revision_number`, and `snapshot_hash` recorded in `PayrollFinalLines.SourceSnapshot`. A locked/archived reader must require one distinct snapshot identity across the period's FinalLines, then load that exact `PayrollCalculationSnapshots` row in the same company/branch/period scope. More than one identity, a missing identity, a missing scoped snapshot, or a revision/hash mismatch is a fail-closed `UNAVAILABLE` provenance result for snapshot-dependent sections. The reader must never select the latest snapshot. This reuses the CP-4F provenance rule already used by CP-5C.

**Immutable-source ownership matrix**

| Finalized-library domain | Immutable authority | Phase 6 treatment |
| --- | --- | --- |
| Final period money, line totals, min/max, and finalized Bonus contribution | `PayrollFinalLines` | Direct finalized financial authority; aggregate set-wise by period/driver. |
| Finalization time and finalizer ID | `PayrollPeriods.LockedAtUtc`/`LockedByUserID`, FinalLines lock fields, and finalization audit event | Reuse IDs/times; new participant evidence supplies immutable display/responsibility context. |
| Snapshot revision, calculation/source hashes, submitted total | `PayrollCalculationSnapshots` header and `PayrollFinalLines.SourceSnapshot` | Reuse after deterministic provenance validation. `SnapshotHash` and `SourceConfigHash` semantics remain unchanged. |
| Frozen driver display identity and submitted driver totals | `PayrollCalculationDriverTotals` and `PayrollPeriodDriverEligibility` snapshots | Reuse snapshot values; do not join current employee/driver names for historical display. |
| Financial line explanation, work date, quantity, resolved amount, rate IDs, behavior, source identity | FinalLines plus originating `PayrollCalculationSnapshotLines` | Reuse; FinalLines wins for money. |
| PayItem code, label, unit, classification, order, and report visibility | `PayrollPeriodPayItems` | Reuse the period snapshot, not current PayItems. Legacy missing metadata is unavailable. |
| Status/off history and status labels | `PayrollCalculationSnapshotStatusEntries` with versioned report-evidence markers | Reuse for evidence-enabled snapshots; no new Status table. |
...
1. `PayrollPeriodWorkflowActionEvidence`: one immutable row for each relevant payroll workflow action. It carries company, branch, period, optional calculation snapshot/revision, optional ReviewItem/decision reference, action code, actor UserID, frozen actor display identity, frozen action-permission/responsibility context, action time, and the already-authoritative reason/decision reference when applicable. It is not a User snapshot and stores no credentials/profile data.
2. `PayrollCalculationSnapshotUsedRateDefinitions`: one immutable, normalized definition row per distinct rate/rule definition actually used by a calculation snapshot. It carries snapshot/company/branch/period identity; a deterministic definition fingerprint; the used PayItem/RateType/DriverRate identities when present; frozen rate/rule label and unit information needed to explain the calculation; resolved value; effective/work-date context; rate behavior; and only the current rule/tier/rounding/block metadata that was actually used. `PayrollCalculationSnapshotLines` gains a nullable reference to this definition for new captures. Existing rows remain legacy and are not backfilled.
3. Seed `ledger.view` and `ledger.audit.view` in the existing `sec.Permissions` catalog. Normal grants continue through the existing company-role/override permission mechanism; Company Owner behavior remains the current dynamic all-catalog rule. No implicit payroll-view grant and no broad default role redesign is introduced.

No new Status-history, Bonus-history, FinalLines, generic operational-source mirror, correction, cache, or full rate-catalog table is required. The finalized Off/Status contract is supportable for new evidence-enabled periods from frozen Status entries, `PayrollPeriodDays`, and eligibility snapshots. A legacy period missing one of those immutable inputs returns that section as unavailable rather than using current schedule, Status, Driver, or DraftLine data.

**Participant evidence design and capture timing:** Use one normalized append-only `PayrollPeriodWorkflowActionEvidence` table rather than parallel Submit/Review/Finalize participant tables. The table has a period-scoped uniqueness constraint appropriate to the action identity: action code plus the authoritative action reference (snapshot revision for Submit/Resubmit, ReviewItem decision reference for Return/Approve, and period finalization action for Finalize). It has scoped foreign keys to the period and, where present, calculation snapshot and ReviewItem; actor UserID remains a provenance FK while frozen display/responsibility values are the historical display authority. Add no-update/no-delete trigger protection using the existing calculation-snapshot/FinalLines immutable-trigger pattern.

- Submit and Resubmit write evidence inside the existing CP-4D `REPEATABLE READ` snapshot/transition transaction, linked to the new snapshot revision.
- Return and Approve write evidence inside the existing review decision transaction, linked to that ReviewItem and its bound snapshot where applicable.
- Finalize writes evidence inside the existing CP-4F finalization transaction, linked to the exact approved snapshot selected for FinalLines.

This captures identity and responsibility when the action is authoritative, not when a reader later opens the Ledger. Resubmission inserts a new action row and a new revision-specific used-rate definition set; it never updates revision 1 evidence.

**Used rate/rule design and capture timing:** Build a canonical used-definition key while CP-4D builds the calculation packet, before snapshot lines are inserted. The key is based only on immutable explanatory facts used by the line: relevant PayItem/RateType/DriverRate identity, resolved value, rate behavior, effective/work-date context, and required current rule/tier/rounding/block metadata. Insert one definition per distinct key for the snapshot, then reference it from every applicable new snapshot line. Direct/manual, Bonus, and system lines with no rate/rule definition retain no reference. The authoritative snapshot is the correct parent because it freezes the calculation packet before mutable rate/rule configuration can drift; CP-4F later selects that same snapshot through FinalLine provenance. This avoids one copied rate configuration per driver/day while preserving line-to-definition provenance for the Rates Used view.

**Legacy availability contract:** Every Phase 6 response exposes a small typed section capability, conceptually `{ state, reason_code }`, where `state` is `AVAILABLE`, `EMPTY`, or `UNAVAILABLE`. `EMPTY` is valid only when a versioned/known capture path exists and has zero rows. `UNAVAILABLE` means immutable truth was never captured or deterministic provenance is absent; use concise machine-readable reasons such as `LEGACY_NOT_CAPTURED` and `PROVENANCE_UNAVAILABLE`. Final money can remain `AVAILABLE` while participant, used-rate, Status, or Bonus descriptive sections are `UNAVAILABLE`. No synthetic backfill is permitted.

**Permission and scope architecture:** All Phase 6 readers use a shared finalized-library access helper before any child data load: require not-driver-role/`OwnDriverDataOnly`, load the period within company scope, require canonical branch access, require `ledger.view`, and require `ledger.audit.view` additionally for audit. The helper returns a locked/archived scoped period context only; all read-model queries bind company, branch, and period identifiers. `reports.view`, `payroll.view`, and payroll-entry permissions are not substitutions for the new finalized-library permissions. Permission seeding is migration work, while route dependencies use the existing `_check_permission`, `_check_branch_access`, and `_require_not_driver_role` conventions.

**Read-model and route architecture:** Add a focused `finalized_library_read_model.py` for period-scoped immutable assembly, plus a thin finalized-library router/service boundary. Keep CP-5C `report_read_model.py` unchanged; reuse its provenance/evidence loaders only through focused shared helpers or extracted immutable primitives, never by treating a Locked period as LIVE. Existing final-line loader code remains reusable for low-level FinalLines queries but its live Driver/Employee display join is not a historical display source.

| Route | Authority and high-level set-based read plan |
| --- | --- |
| `GET /payroll/finalized/{period_id}/overview` | Scoped locked/archived period plus finalization provenance; one FinalLines aggregate for total/count/driver count; section availability derived from provenance/version markers. No child collections. |
| `GET /payroll/finalized/{period_id}/reports/{view}` | FinalLines for money; originating snapshot DriverTotals/SnapshotLines/PeriodPayItems for frozen explanation and columns; Status/Bonus evidence only from that snapshot. Map only the four established views: `drivers`, `period-work`, `period-pay`, `mixed`. |
| `GET /payroll/finalized/{period_id}/off-drivers` | Originating snapshot Status evidence, `PayrollPeriodDays`, and period driver-eligibility snapshots in batched period reads. Missing frozen calendar/eligibility/evidence yields section unavailability, not current schedule reconstruction. |
| `GET /payroll/finalized/{period_id}/rates-used` | Originating snapshot used-rate definitions joined to snapshot-line references; return a compact ordered definition set with use references/counts and driver/PayItem linkage supplied from immutable rows. Existing legacy rate provenance can be shown only where its immutable fields suffice; otherwise expose `UNAVAILABLE`. |
| `GET /payroll/finalized/{period_id}/audit` | Completed P6D route: period-scoped immutable P6D event/coverage/membership evidence plus immutable workflow participant evidence and exact finalized provenance. Missing legacy immutable facts are explicitly unavailable; current mutable rows are never used to fill them. |

All routes lazy-load their own child domain. They batch by period/snapshot and use joins, aggregates, or `IN`/CTE sets rather than queries per driver, date, Status, Bonus, rate, audit row, or report cell. Proposed indexes are only: participant evidence `(CompanyID, BranchID, PayrollPeriodID, ActionAtUtc)` and unique action identity; used-rate definitions `(PayrollCalculationSnapshotID, DefinitionFingerprint)` and snapshot-line used-definition reference; and any missing foreign-key indexes required by the final DDL. Existing period FinalLines, snapshot, Status/Bonus evidence, PeriodPayItems, PeriodDays, and eligibility indexes cover the planned primary reads. Phase 7 owns load-test budgets and broad performance validation.

**Immutability, tenant integrity, and caching:** New evidence rows carry CompanyID, BranchID, and PayrollPeriodID and use scoped snapshot/period foreign keys or repository-consistent ownership triggers to prevent cross-tenant references. New tables receive no-update/no-delete database triggers. Writer services validate the snapshot/ReviewItem/period share the same company/branch before insert. Finalized response ETags or future application cache keys may use the locked period ID plus FinalLine provenance snapshot hash/revision and relevant evidence version; caching is optional and is not required for correctness.

**Migration and transaction plan:** Migration `0064` is required for the additive evidence tables, snapshot-line used-definition reference, permission catalog rows, scoped foreign keys/uniqueness, immutable triggers, and only the indexes above. It contains no historical backfill and no change to `SnapshotHash`, `SourceConfigHash`, `ReportEvidenceHash`, or FinalLines. CP-4D captures used-rate definitions atomically with header, lines, Status/Bonus evidence, and ReviewItem linkage. Review capture and CP-4F finalization capture occur atomically with their respective authoritative transitions. A failed action rolls back its evidence with the action, preventing partial historical records.

**Correction boundary:** P6F requires no production correction persistence in Phase 6. The library may expose the immutable original period identity and the future-boundary capability, but it creates no correction record, workflow, calculation, posting, or reopen path.

**Focused test architecture:** Add focused Phase 6 tests for `ledger.view`, additive `ledger.audit.view`, company/branch scope, driver/ODA denial, lightweight overview, lazy child routes, FinalLines-only money, no LIVE recalculation, provenance fail-closed behavior, archived parity, frozen Status/Bonus, legacy `UNAVAILABLE`, versioned zero-row `AVAILABLE`/`EMPTY`, and partial legacy responses. Evidence-capture tests must prove a user rename/role change cannot alter participant display/responsibility, a rate/rule mutation cannot alter a used definition, and resubmission preserves revision-specific evidence. Query characterization should assert period-batched behavior where the repository test harness can observe it, without imposing a fixed production query-count SLA.
...
5. **P6D — Complete:** lazy finalized audit/security detail using dedicated immutable period evidence, participant evidence, and immutable workflow facts.
6. **P6E/P6F — Deferred:** retain their existing participant-read and never-reopen/future-linked-correction planning boundaries; do not implement either contract or the future correction workflow.
...
Phase 5 remains `Complete`. Phase 6 is `In Progress`: the internal foundation, P6A, P6B, P6C, and P6D are complete; P6E and P6F are deferred. Phase 7 remains `Pending` and deferred, retaining broad append-only audit hardening, constraint validation, permission cleanup, load testing, migration rehearsal, reconciliation, and rollout.
