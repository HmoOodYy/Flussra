...
Existing snapshots may predate this evidence. They require no synthetic backfill and remain reportable where their authoritative data is present, but any missing historical Status or Bonus field must be explicitly represented as unavailable, distinguishable from a legitimate null or zero value. Missing optional historical evidence alone does not fail the entire report unless an existing integrity rule requires failure.
...
**CP-5C readiness blocker and approved direction:** Discovery found that complete frozen report evidence is not yet persisted for historical Status summaries and Bonus giver/creator-plus-timestamp detail. The approved direction is to remedy snapshot persistence before full CP-5C implementation. This requirement applies to new snapshots after the remedy; legacy snapshots remain readable with explicit unavailable evidence markers and no synthetic historical truth. The implementation design remains intentionally open: this plan does not choose a table versus JSONB, migration shape, endpoint status for unavailable reports, or legacy unavailable-field JSON shape.
...
| CP-5C | Calculation report bundle — **Pending: frozen report-evidence persistence remedy** | payroll report API and tests | frontend sums, mutable source reconstruction for frozen reports, or implementation design before review | dynamic columns, frozen Status/Bonus evidence, and Drivers/Work/Pay/Mixed parity | All required report totals and historical evidence are backend-owned; legacy unavailable evidence is explicit rather than invented |
