...
**Implementation status:** these routes remain planned and are not implemented. RP-1 (`187d552e00ff26f2bd55015c8072a861e93cca7e` — `feat: add payroll report authority resolver`) now provides the internal lifecycle financial-authority resolver future report services must use. Frontend clients must never choose live vs. snapshot vs. FinalLines authority. CP-5C is pending a frozen report-evidence persistence remedy before implementation.
...
**CP-5C frozen-report evidence decisions (pending implementation):** New Submit/Resubmit calculation snapshots must preserve immutable Status evidence sufficient for historical status reporting, and immutable Bonus evidence sufficient for Drivers Report detail: amount, giver/creator identity, event timestamp, reason, and notes where applicable. Status remains a separate source channel, not a PayItem; Bonus remains the canonical Bonus-event domain, distinct from generic manual financial rows. Frozen report states must not rebuild those historical fields from mutable current driver-day Status or BonusEvent rows.

**Approved persistence architecture: dedicated immutable report-evidence tables.** CP-5C will use dedicated immutable report-evidence tables attached to each calculation snapshot, conceptually `PayrollCalculationSnapshotStatusEntries` and `PayrollCalculationSnapshotBonusEvents` (implementation may make minor repository-consistent naming adjustments). It will not use snapshot-level JSON arrays as the primary evidence model and will not overload financial `PayrollCalculationSnapshotLines` with non-financial Status evidence.
...
- Calculation snapshot headers will gain `ReportEvidenceVersion` and `ReportEvidenceHash`. These markers distinguish legacy snapshots for which report evidence was never captured from new snapshots with successful capture but zero Status and/or Bonus rows, version future evidence-schema evolution, and provide deterministic report-evidence integrity verification. `ReportEvidenceHash` is based on deterministically ordered canonical serialization: identical captured evidence produces the same hash and different evidence produces a different hash.
...
This architecture requires a schema migration, with Alembic currently at `0062`; implementation will use the next valid revision after `0062` if no intervening migration exists. No migration or historical backfill is created by this decision record. The persistence remedy itself remains unimplemented, so CP-5C remains pending/blocked until implementation is reviewed and committed.
...
**CP-5C readiness blocker and approved direction:** Discovery found that complete frozen report evidence is not yet persisted for historical Status summaries and Bonus giver/creator-plus-timestamp detail. The approved direction is a dedicated immutable report-evidence table remedy attached to each calculation snapshot, with `ReportEvidenceVersion`/`ReportEvidenceHash` header markers, before full CP-5C implementation. This requirement applies to new snapshots after the remedy; legacy snapshots remain readable with explicit unavailable evidence markers and no synthetic historical truth. The persistence architecture is resolved; the migration shape, unavailable endpoint behavior, and legacy unavailable-field JSON shape remain implementation details subject to the locked decisions above.

**Execution order:** CP-5C remains pending and, after the frozen report-evidence persistence remedy is designed and implemented, begins the calculation-report bundle under official P5D/P5E, reusing RP-1 rather than recreating authority selection. Drivers Report, Period Work, Period Pay, and Mixed/Work+Pay views remain pending. Do not start RP-2 Period Pay before CP-5C.
...
| CP-5C | Calculation report bundle — **Pending: frozen report-evidence persistence remedy** | payroll report API and tests | frontend sums, mutable source reconstruction for frozen reports, or report implementation before the approved evidence remedy | dynamic columns, frozen Status/Bonus evidence, and Drivers/Work/Pay/Mixed parity | All required report totals and historical evidence are backend-owned; legacy unavailable evidence is explicit rather than invented |
